  ...
  render: =>
    <View>
      <Error error={@props.validation.phoneNumber.error}>
        <View>
          <Input maxLength={11} type="number" placeholder="휴대폰번호" state={@props.state} path="phoneNumber" />
      ...
      <Error error={@props.state.secretNumberError}>
        <View>
          <Input type="number" placeholder="인증번호" state={@props.state} path="secretNumber" />
          <View style={{ position: 'absolute', right: 0, top: 4 }}>
