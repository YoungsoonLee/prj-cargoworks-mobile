    ...
    <Layout title="충전금 환급 요청">
      <ScrollView paddingBottom={400}>
        ...
        <View style={{ paddingHorizontal: 20, paddingVertical: 30 }}>
          ...
          <Error error={@props.validation.amount.error}>
            <Input type="number" marginTop={30} placeholder="금액 입력" state={@props.state} path="amount" />
          </Error>
        </View>
      </ScrollView>
      <Button isDisabled={not @props.validation.isValid} borderRadius={0} height={75} color="light blue" onPress={@onPressConfirm}>환급 요청하기</Button>
    </Layout>
